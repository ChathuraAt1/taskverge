<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" class="dark h-full bg-slate-900 text-slate-100 antialiased selection:bg-emerald-500 selection:text-white">
<head>
    <meta charset="utf-8">
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="rounded-lg border border-slate-800 bg-slate-900/60 p-3">
                <div class="flex items-center justify-between text-xs">
                    ...
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-[11px] font-semibold uppercase tracking-wider
                        @if(auth()->user()->isSuperAdmin()) bg-amber-500/20 text-amber-300 border border-amber-500/30
                        @elseif(auth()->user()->isAdmin()) bg-purple-500/20 text-purple-300 border border-purple-500/30
                        @elseif(auth()->user()->isManager()) bg-teal-500/20 text-teal-300 border border-teal-500/30
                        @elseif(auth()->user()->isOperator()) bg-emerald-500/20 text-emerald-300 border border-emerald-500/30
            ...
            <nav class="flex flex-col gap-1 text-sm font-medium">
                ...
                <!-- AI Copilot -->
                <a href="{{ route('copilot') }}" class="flex items-center justify-between rounded-lg px-3 py-2 transition-colors {{ request()->routeIs('copilot') ? 'bg-emerald-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }}">
                    ...
                    <span class="rounded-full bg-emerald-400/20 text-emerald-300 border border-emerald-400/30 text-[9px] px-1.5 py-0.2 font-extrabold uppercase tracking-wide">
                        AI
                    </span>
                </a>

                <a href="{{ route('workflows.index') }}" class="flex items-center gap-3 rounded-lg px-3 py-2 transition-colors {{ request()->routeIs('workflows.*') ? 'bg-emerald-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <a href="{{ route('tasks.index') }}" class="flex items-center gap-3 rounded-lg px-3 py-2 transition-colors {{ request()->routeIs('tasks.*') ? 'bg-emerald-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <!-- Analytics & Telemetry -->
                <a href="{{ route('analytics') }}" class="flex items-center gap-3 rounded-lg px-3 py-2 transition-colors {{ request()->routeIs('analytics') ? 'bg-emerald-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <!-- Team Hub -->
                <a href="{{ route('team') }}" class="flex items-center gap-3 rounded-lg px-3 py-2 transition-colors {{ request()->routeIs('team') ? 'bg-emerald-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                <!-- Audit Log -->
                <a href="{{ route('audit') }}" class="flex items-center gap-3 rounded-lg px-3 py-2 transition-colors {{ request()->routeIs('audit') ? 'bg-emerald-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h3.75M9 15h3.75M9 18h3.75m3 .75H18a2.25 2.25 0 002.25-2.25V6.108c0-1.135-.845-2.098-1.976-2.192a48.424 48.424 0 00-1.123-.08m-5.801 0c-.065.21-.1.433-.1.664 0 .414.336.75.75.75h4.5a.75.75 0 00.75-.75 2.25 2.25 0 00-.1-.664m-5.8 0A2.251 2.251 0 0113.5 2.25H15c1.012 0 1.867.668 2.15 1.586m-5.8 0c-.376.023-.75.05-1.124.08C9.095 4.01 8.25 4.973 8.25 6.108V8.25m0 0H4.875c-.621 0-1.125.504-1.125 1.125v11.25c0 .621.504 1.125 1.125 1.125h9.75c.621 0 1.125-.504 1.125-1.125V9.375c0-.621-.504-1.125-1.125-1.125H8.25zM6.75 12h.008v.008H6.75V12zm0 3h.008v.008H6.75V15zm0 3h.008v.008H6.75V18z" />
                    </svg>
                    Audit Trail
                ...
                <a href="{{ route('admin.panel') }}" class="flex items-center gap-3 rounded-lg px-3 py-2 transition-colors {{ request()->routeIs('admin.*') ? 'bg-emerald-600 text-white font-semibold' : 'text-slate-300 hover:bg-slate-900 hover:text-white' }}">
                    <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                ...
                @foreach($demoAccounts as $acc)
                    @if($acc->id !== auth()->id())
                        <form method="POST" action="{{ route('quick-login', $acc) }}">
                            @csrf
                            <button type="submit" class="w-full flex items-center justify-between rounded-md bg-slate-900/80 px-2.5 py-1.5 text-xs text-slate-300 hover:bg-emerald-600 hover:text-white transition-colors">
    ...
    <div class="flex flex-1 flex-col overflow-hidden">
        ...
        <header class="flex h-16 items-center justify-between border-b border-slate-800 bg-slate-950/80 px-4 sm:px-6 lg:px-8">
            <div class="flex items-center gap-4">
                ...
                <h1 class="text-lg font-bold text-white tracking-tight">{{ $header ?? 'TaskVerge Autonomous Intelligence' }}</h1>
            </div>
            ...
            <div class="flex items-center gap-3">
                <div class="hidden sm:flex items-center gap-2 rounded-full border border-emerald-500/30 bg-emerald-500/10 px-3 py-1 text-xs text-emerald-400">
                    ...
                    <span class="font-medium">Live Telemetry Active</span>
                </div>
        ...
        @if (session('status'))
            <div class="bg-emerald-600/90 text-white px-4 py-2 text-xs font-medium flex items-center justify-between">
