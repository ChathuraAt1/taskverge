    ...
    <div class="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-3 bg-slate-950/60 p-3 rounded-xl border border-slate-800">
        ...
        <div class="flex flex-wrap items-center gap-2">
            <select wire:model.live="workflowFilter"
                    class="rounded-lg border border-slate-700 bg-slate-900 px-2.5 py-2 text-xs text-slate-200 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 max-w-[180px]">
                    ...
                    @forelse($tasks as $task)
                        <tr class="hover:bg-slate-900/50 transition-colors">
                            ...
                            <td class="px-4 py-3 max-w-xs">
                                <button wire:click="inspectTask({{ $task->id }})" class="font-medium text-slate-100 hover:text-emerald-400 transition-colors text-left line-clamp-1">
                                    {{ $task->title }}
                                </button>
                            ...
                            <td class="px-4 py-3">
                                <a href="{{ route('workflows.show', $task->workflow) }}" class="text-slate-300 hover:text-emerald-300 transition-colors truncate block max-w-[150px]">
                                    {{ $task->workflow->title }}
                                </a>
                            </td>
                            <td class="px-4 py-3">
